...
var renderReliquary = function renderReliquary(tokenArray) {
	...
	deckNames.forEach(function (name) {
		...
		if (deckChanceList[name].average > 0) {
			var _source = tpl.nav.source.new();
			_source._header._name.textContent = name;
			_source._header._average.textContent = Math.round(deckChanceList[name].average);
			_source._details.prepend(_source._header.cloneNode(true));
			// Over rotations with any relics
			deckChanceList[name].rotations.forEach(function (v, rotationName) {
				if (v.sum > 0) {
					var _rotation = tpl.nav.rotation.new();
					_rotation._header._name.textContent = L.text_rotation + " " + rotationName;
...
var renderActiveFissures = function renderActiveFissures(worldStateData) {
	// Clear node state
...
worldState.stayFresh = function () {
	...
	|| LS[timeKey] < Date.now() - 60000) {
		...
		loadJsonCache(stateKey).then(function (data) {
			...
			renderActiveFissures(data);
		});
	} else {
		// Valid state stored
		if (LS[stateKey]) {
			renderActiveFissures(JSON.parse(LS[stateKey]));
		}
